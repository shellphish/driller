...
def in_catalogue(var_cnt, prev_addr, next_addr):
    ...
    :param var_cnt: number of variables involved in the generation
    ...
    if (var_cnt, prev_addr, next_addr) in catalogue_cache:
        return True
    ...
    if sz != catalogue_last_size:
        ...
        for entry in catalogue_blob.split("\n")[:-1]:
            vcnt, paddr, naddr = map(lambda x: int(x, 16), entry.split(","))
            catalogue_cache.add((vcnt, paddr, naddr))
    ...
    catalogue_cache.add((var_cnt, prev_addr, next_addr))
